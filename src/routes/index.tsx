...
import { runLab } from "@/lib/camase/runner";
...
function LabPage() {
  ...
  const result = useMemo(() => {
    void runId;
    return runLab(n, track, seed, gated);
  }, [n, track, seed, gated, runId]);

  const ready = result.series.filter((s) => s.ready);
  ...
  const chart = ready.filter((_, i) => i % 2 === 0).map((s) => ({
    t: s.t,
    obs: s.y,
    filt: s.pHat,
    latent: s.latent,
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel title="Log-price" className="lg:col-span-2">
          ...
          <p className="mt-2 text-xs text-fg-subtle">
            Paper · latent · filter. Warm-up discarded. Length {n} · seed {seed}.
          </p>
        </Panel>

        <Panel title="Last sample">
          <dl className="space-y-2 font-mono text-sm tabular-nums">
            <Row k="ŷ / p̂" v={`${last ? last.y.toFixed(5) : "—"} / ${last ? last.pHat.toFixed(5) : "—"}`} />
          ...
          <div className="mt-4 flex gap-2">
            <button
              type="button"
              className="h-11 flex-1 rounded-[10px] border border-line text-sm text-fg-muted"
              onClick={() => setN((x) => (x === 800 ? 1200 : x === 1200 ? 1800 : 800))}
            >
              N = {n}
            </button>
          </div>
